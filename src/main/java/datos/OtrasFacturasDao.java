...
import dominio.otrasfactuas.Cabecera;
import dominio.otrasfactuas.ConceptoRepercutible;
import dominio.otrasfactuas.DocumentoOtraFactura;
import dominio.otrasfactuas.RegistroFin;
import java.sql.*;
import java.util.*;
...
public class OtrasFacturasDao {
    ...
    private static final String SQL_SELECT_BY_COD_FISCAL = " where cod_fis_fac = ? or cod_fis_fac = ? AND is_deleted = 0";

    //Contenido XML por registro
    private static final String SQL_SELECT_BY_ID_CLIENTE = "SELECT * FROM contenido_xml_otras_facturas where id_cliente = ? AND is_deleted = 0 order by idcontenido_xml desc";
    ...
    private static final String SQL_INSERT = "("
            + "cod_emp_emi, cod_emp_des, cod_pro, cod_pas, cod_sol, cups, " //cabecera
    ...
    public List<DocumentoOtraFactura> listar() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                //Comentarios
                StringBuilder comentarios = new StringBuilder(rs.getString("com_dev"));

                //Errores
                StringBuilder errores = new StringBuilder(rs.getString("id_err"));
